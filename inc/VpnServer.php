...
class VpnServer {
    ...
    private function testConnection(): bool {
        $testCommand = sprintf(
            "sshpass -p '%s' ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o PreferredAuthentications=password -o PubkeyAuthentication=no -o ConnectTimeout=10 -p %d %s@%s 'echo test' 2>/dev/null",
            $this->data['password'],
            $this->data['port'],
            $this->data['username'],
            $this->data['host']
